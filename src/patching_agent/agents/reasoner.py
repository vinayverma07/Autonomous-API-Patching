"""
Module: Reasoning Agent Node
Description: Uses a local Mistral model to execute structural analysis and 
             generate a structured repair blueprint without generating raw code yet.
"""
...
class RepairBlueprint(BaseModel):
    """The structured output schema generated by our Reasoning Agent."""
...
class ReasoningAgentNode:
    """Agent node that orchestrates code analysis and plans the repair strategy."""

    def __init__(self):
        logger.info("Initializing Reasoning Agent Node with Mistral...")
        self.llm = Ollama(
    ...
    def _build_reasoning_prompt(self, state: AgentGraphState) -> str:
        """Assembles the comprehensive engineering prompt including logs, source code, and RAG context."""
...
Your response must contain ONLY a single valid JSON object. Do not include any conversational text, markdown formatting blocks (like ```json), or code patches yet. Focus completely on the architectural planning.
...
"""
    ...
    async def reason(self, state: AgentGraphState) -> Dict[str, Any]:
        """Executes the planning and analysis logic, saving the plan to the graph state."""
        logger.info("Reasoning Agent analyzing code conflicts and building repair plan...")

        if not state.original_code.strip():
            logger.warning("Original target source code is missing from the state tree. Skipping reasoning.")
            return {
        ...
        try:
            ...
            # Clean up accidental markdown wrappers if present
            ...
            logger.error(f"Reasoning model output could not be parsed as valid JSON: {je}. Raw: {response_text}")
            return {"execution_status": "reasoning_parsing_failed"}
        except Exception as e:
            logger.error(f"Unexpected error in Reasoning Agent Node: {e}")
            return {"execution_status": "reasoning_node_failed"}

if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO)
    node = ReasoningAgentNode()
    print("Reasoning Agent module compiled successfully.")
